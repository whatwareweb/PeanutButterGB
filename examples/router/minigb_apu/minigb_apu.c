...
static void update_square(struct minigb_apu_ctx *ctx, audio_sample_t *samples,
		const bool ch2, const uint_fast16_t num_samples)
{
	...
	for (uint_fast16_t i = 0; i < num_samples; i += 2) {
		update_len(ctx, c);
		if (!c->enabled)
			return;
...
static void update_wave(struct minigb_apu_ctx *ctx, audio_sample_t *samples, const uint_fast16_t num_samples)
{
...
static void update_noise(struct minigb_apu_ctx *ctx, audio_sample_t *samples, const uint_fast16_t num_samples)
{
...
{
	memset(stream, 0, frameCount * sizeof(audio_sample_t) * 2);
	update_square(ctx, stream, 0, frameCount * 2);
	update_square(ctx, stream, 1, frameCount * 2);
	update_wave(ctx, stream, frameCount * 2);
	update_noise(ctx, stream, frameCount * 2);
}
